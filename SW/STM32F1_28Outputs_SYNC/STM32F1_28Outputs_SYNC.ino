 ...
 *                  - Mettre tous les GPIO en INPUT_PULLDOWN à l’initialisation                                   DONE
...
void setup() {
  ...
  { // initialize ALL GPIO pins to INPUT_PULLDOWN in order to limit interrupts on floating pins
    int pinNumber = 0;
    while ( GPIOINIT_TABLE[pinNumber] != 0xffff ) pinMode(GPIOINIT_TABLE[pinNumber++], INPUT_PULLDOWN);
  }
